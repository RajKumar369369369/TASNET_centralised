...
def save_checkpoint(state, fpath='checkpoint.pth.tar'):
    ...
    The 'state' dict should include: 'state_dict', 'optimizer', 'epoch', and 'args'.
    """
...
def load_checkpoint(fpath, model, optimizer=None):
    """
    Loads a checkpoint. If the checkpoint is from the old REINFORCE model,
    strict=False ensures it doesn't crash when the Critic weights are missing.
    """
    if not osp.exists(fpath):
        raise FileNotFoundError(f"No checkpoint found at '{fpath}'")
    
    checkpoint = torch.load(fpath, map_location=lambda storage, loc: storage)
    
    # strict=False is key for migrating from REINFORCE to PPO
    model.load_state_dict(checkpoint['state_dict'], strict=False)
    
    if optimizer is not None and 'optimizer' in checkpoint:
        optimizer.load_state_dict(checkpoint['optimizer'])
        
    print(f"Loaded checkpoint from {fpath} (epoch {checkpoint.get('epoch', 'unknown')})")
    return checkpoint
