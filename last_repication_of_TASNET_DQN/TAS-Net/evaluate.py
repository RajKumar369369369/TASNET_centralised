...
def evaluate(args, model1, model2, dataset, test_keys):
    ...
    with torch.no_grad():
        ...
        for key_idx, key in enumerate(test_keys):
            ...
            if hasattr(args, 'rl_algo') and args.rl_algo != 'ppo':
                q_values = model2(seq_graph0)                             # (1, T, 2)
                # Use softmax select-probability as the importance score.
                # This preserves relative ranking scale just like the baseline DSN sigmoid output.
                probs_importance = torch.softmax(q_values, dim=-1)[0, :, 1].data.cpu().numpy()  # (T,)
            else:
                res = model2(seq_graph0)
