...
def compute_incremental_reward(seq, actions, ignore_far_sim=False, temp_dist_thre=20):
    """
    Computes an incremental reward for DQN:
    r_t = +1 if adding frame t improves overall cluster quality (diversity + representativeness),
    r_t = -1 if adding frame t degrades it (noisy/redundant).
    """
    _seq = seq.detach().squeeze()
    _actions = actions.detach().squeeze()
    n = _seq.size(0)
    ...
    current_actions = torch.zeros(n, device=device)
    prev_quality = 0.0
    
    for t in range(n):
        if _actions[t] > 0:
            current_actions[t] = 1
            # Compute reward for the subset {0..t}
            dense_res = compute_reward(_seq.unsqueeze(0), current_actions.unsqueeze(0), ignore_far_sim, temp_dist_thre)
            # The quality is the value assigned to the picked indices
            curr_quality = dense_res[t].item() if dense_res[t] != 0 else 0.0
            
            delta = curr_quality - prev_quality
            step_rewards[t] = 1.0 if delta > 0 else -1.0
            prev_quality = curr_quality
        else:
            step_rewards[t] = 0.0
            
    return step_rewards.unsqueeze(1)
