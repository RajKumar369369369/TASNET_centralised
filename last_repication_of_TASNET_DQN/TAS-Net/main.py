...
# DQN Hyperparameters
parser.add_argument('--rl_algo', type=str, default='ppo', choices=['ppo', 'dqn', 'ddqn', 'dueling_dqn', 'd3qn'])
...
if __name__ == '__main__':
    if args.training:
         ...
         if args.rl_algo == 'ppo':
             ...
             use_double = (args.rl_algo in ['ddqn', 'd3qn'])
             use_dueling = (args.rl_algo in ['dueling_dqn', 'd3qn'])
             agent = DQNAgent(in_dim=args.n_feature, hid_dim=args.hid_dim, num_layers=1, cell='gru',
                              ...
                              use_double=use_double, use_dueling=use_dueling, use_per=args.use_per, n_step=args.n_step)
             Network2 = agent.online_net
             optimizer = optim.Adam(
                list(Network1.parameters()) + list(Network2.parameters()),
                lr=args.lr,
                weight_decay=args.weight_decay
         ...
         print(f"=====> Start {args.rl_algo.upper()} Training <=====")
             ...
             for key in train_keys:
                ...
                # =========================
                 if args.rl_algo == 'ppo':
                     ...
                     # ================================================================
                     # DQN Variants — reusing original TAS-Net reward semantics
                     # ================================================================

                     # Forward pass through DQN to get Q-values and hidden states
                     q_values, h_seq = Network2(seq_graph, return_hidden=True)
                     h_seq_detached = h_seq.detach()

                     # Epsilon-greedy action selection using GRU hidden state as state
                     actions = agent.select_action(h_seq_detached, training=True)  # (1, T)
                     actions = actions.t()  # (T, 1)

                     # ---------------------------------------------------------------
                     # REWARD: Reuse the ORIGINAL TAS-Net reward exactly.
                     # Treat one EEG sequence as one episode. Compute a single scalar
                     # global reward (diversity + representativeness) and assign it
                     # to every transition in the episode. This preserves TAS-Net
                     # training semantics and prevents Q-value collapse.
                     # ---------------------------------------------------------------
                     step_rewards_raw = compute_reward(seq_graph, actions)  # (T,)
                     episode_reward = step_rewards_raw.mean().item()         # scalar
                     T_len = h_seq.size(1)

                     # ---------------------------------------------------------------
                     # ONLINE LOSS: Compute TD target using episode-level reward.
                     # done is True ONLY at the last timestep (t == T-1).
                     # ---------------------------------------------------------------
                     q_val = q_values.squeeze(0).gather(-1, actions).squeeze(-1)  # (T,)
                     with torch.no_grad():
                         next_q_values = agent.target_net(seq_graph, return_hidden=False).squeeze(0)  # (T, 2)
                         if agent.use_double:
                             next_actions = q_values.squeeze(0).argmax(dim=-1, keepdim=True)
                             next_q_val = next_q_values.gather(-1, next_actions).squeeze(-1)
                         else:
                             next_q_val = next_q_values.max(dim=-1)[0]

                     target_q = torch.zeros(T_len, device=DEVICE)
                     for t in range(T_len):
                         next_idx = min(t + 1, T_len - 1)
                         done = 1.0 if t == T_len - 1 else 0.0  # done only at final step
                         target_q[t] = episode_reward + args.gamma * next_q_val[next_idx] * (1 - done)

                     td_loss = nn.SmoothL1Loss()(q_val, target_q.detach())

                     # ---------------------------------------------------------------
                     # BALANCE LOSS: Prevent all-discard / all-select collapse.
                     # Penalise selection ratio deviating from 50%.
                     # ---------------------------------------------------------------
                     select_probs = torch.softmax(q_values.squeeze(0), dim=-1)[:, 1]  # (T,)
                     selection_ratio = select_probs.mean()
                     balance_loss = 0.01 * (selection_ratio - 0.5).pow(2)

                     online_loss = td_loss + balance_loss
                     ...
                     # ---------------------------------------------------------------
                     # REPLAY BUFFER: Store (hidden_state, action, episode_reward,
                     # next_hidden_state, done) as lightweight CPU numpy arrays.
                     # ---------------------------------------------------------------
                     h_seq_cpu = h_seq_detached.squeeze(0).cpu().numpy()  # (T, hid*2)
                     actions_cpu = actions.squeeze(-1).cpu().numpy()       # (T,)
                     for t in range(T_len):
                         next_idx = min(t + 1, T_len - 1)
                         done = 1.0 if t == T_len - 1 else 0.0
                         agent.memory.push(h_seq_cpu[t], actions_cpu[t], episode_reward, h_seq_cpu[next_idx], done)

                     # Off-policy updates from replay buffer
                     for _ in range(4):
                         agent.update()

                     rewards = step_rewards_raw  # for logging
