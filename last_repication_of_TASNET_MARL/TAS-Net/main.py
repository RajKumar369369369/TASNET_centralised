...
parser.add_argument('--save_path', type=str, default='d:\PycharmProjects\TAS-Net\TAS-output\SEED\session1', help='output directory and fragments')
...
# ── MARL arguments ────────────────────────────────────────────────────────────
parser.add_argument('--use_marl', action='store_true', default=False,
                    help='Enable Independent Multi-Agent PPO (MARL) mode.')
parser.add_argument('--num_agents', type=int, default=4,
                    help='Number of independent agents for MARL (default: 4).')
parser.add_argument('--marl_div_weight', type=float, default=0.001,
                    help='Weight for agent-diversity regularization (default: 0.001).')
# ─────────────────────────────────────────────────────────────────────────────
...
if __name__ == '__main__':
    if args.training:
        ...
        marl_net = None
        if args.use_marl:
            marl_net = MARLAgents(hid_dim=args.hid_dim,
                                  num_agents=args.num_agents,
                                  device=DEVICE)
            print("[MARL] Enabled with {} agents.".format(args.num_agents))
            print("Model size (MARL heads): {:.5f}M".format(
                sum(p.numel() for p in marl_net.parameters()) / 1000000.0))
        # ─────────────────────────────────────────────────────────────────────
        ...
        if marl_net is not None:
            parameter_list += [{'params': marl_net.parameters(), 'lr_mult': 10, 'decay_mult': 2}]
        ...
        if marl_net is not None:
            marl_net = marl_net.to(DEVICE)

        if args.use_marl:
            print("=====> Start MARL-PPO training and Online Evaluate <=====")
        else:
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            for idx in idxs:
                ...
                # ── MARL-aware forward ────────────────────────────────────────
                if marl_net is not None:
                    # Run DSN RNN to get hidden states; agents re-weight them
                    dsn_hidden, _ = Network2.rnn(seq)   # (1, seq_len, hid_dim*2)
                    sig_probs = marl_net(dsn_hidden)    # (1, seq_len, 1)
                else:
                    # Baseline: original DSN forward
                    sig_probs = Network2(seq)
                # ─────────────────────────────────────────────────────────────

                cost = 0.01 * (sig_probs.mean() - 0.5) ** 2

                # Tiny diversity regularization (MARL only, weight 0.001)
                if marl_net is not None:
                    cost = cost + args.marl_div_weight * marl_net.diversity_loss(None)
            ...
            if (epoch + 1) % 1 == 0:
                ...
                if Recall > best_recall:
                    ...
                    suffix = ('_marl' if args.use_marl else '') + '_subject' + str(args.subject_id) + '_' + str(args.reward_function)
                    model_save_path1 = osp.join(args.save_path,
                                                'pretrained_best_model1_seed_' + suffix + '.pth.tar')
                    model_save_path2 = osp.join(args.save_path,
                                                'pretrained_best_model2_seed_' + suffix + '.pth.tar')
                    ...
                    # Save MARL heads checkpoint alongside if enabled
                    if marl_net is not None:
                        model_save_path_marl = osp.join(args.save_path,
                            'pretrained_best_marl_seed_' + suffix + '.pth.tar')
                        save_checkpoint(marl_net.state_dict(), model_save_path_marl)
                        print("[MARL] Heads saved to {}".format(model_save_path_marl))
        ...
        model_state_dict1 = Network1.state_dict()
        model_state_dict2 = Network2.state_dict()
        suffix_final = ('_marl' if args.use_marl else '') + '_' + str(args.reward_function)
        model_save_path1 = osp.join(args.save_path, 'pretrained_model1_seed_' + suffix_final + '.pth.tar')
        model_save_path2 = osp.join(args.save_path, 'pretrained_model2_seed_' + suffix_final + '.pth.tar')
        save_checkpoint(model_state_dict1, model_save_path1)
        save_checkpoint(model_state_dict2, model_save_path2)
        print("Model saved to {}".format(model_save_path1))
        print("Model saved to {}".format(model_save_path2))
        if marl_net is not None:
            model_save_path_marl_final = osp.join(args.save_path, 'pretrained_marl_seed_' + suffix_final + '.pth.tar')
            save_checkpoint(marl_net.state_dict(), model_save_path_marl_final)
            print("[MARL] Heads saved to {}".format(model_save_path_marl_final))
        ...
        # ── checkpoint suffix mirrors training ──
        infer_suffix = ('_marl' if args.use_marl else '') + '_subject' + str(args.subject_id) + '_' + str(args.reward_function)
        checkpoint_path1 = osp.join(args.save_path, 'pretrained_best_model1_seed_' + infer_suffix + '.pth.tar')
        checkpoint1 = torch.load(checkpoint_path1, map_location=DEVICE)
        model1.load_state_dict(checkpoint1)
        model1 = model1.to(DEVICE)
        checkpoint_path2 = osp.join(args.save_path, 'pretrained_best_model2_seed_' + infer_suffix + '.pth.tar')
        checkpoint2 = torch.load(checkpoint_path2, map_location=DEVICE)
        model2.load_state_dict(checkpoint2)
        model2 = model2.to(DEVICE)
        # ── Load MARL heads if requested ──
        marl_net_infer = None
        if args.use_marl:
            marl_net_infer = MARLAgents(hid_dim=args.hid_dim,
                                        num_agents=args.num_agents,
                                        device=DEVICE)
            checkpoint_path_marl = osp.join(args.save_path, 'pretrained_best_marl_seed_' + infer_suffix + '.pth.tar')
            checkpoint_marl = torch.load(checkpoint_path_marl, map_location=DEVICE)
            marl_net_infer.load_state_dict(checkpoint_marl)
            marl_net_infer = marl_net_infer.to(DEVICE)
            print("[MARL] Loaded agent heads from {}".format(checkpoint_path_marl))
        with torch.no_grad():
            model1.eval()
            model2.eval()
            if marl_net_infer is not None:
                marl_net_infer.eval()
            ...
            for key_idx, key in enumerate(test_keys):
                ...
                # ── MARL-aware forward (inference) ──
                if marl_net_infer is not None:
                    dsn_hidden_infer, _ = model2.rnn(seq2seq)   # (1, seq_len, hid_dim*2)
                    sig_probs = marl_net_infer(dsn_hidden_infer) # (1, seq_len, 1)
                else:
